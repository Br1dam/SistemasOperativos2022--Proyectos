...
void fat_file_eliminate(fat_file file, fat_file parent) {
    u32 last_cluster = file->start_cluster, next_cluster = 0;

    // Mark following clusters as not used
    while (!fat_table_cluster_is_EOC(
        last_cluster)) { // if last_cluster is the end of the cluster chain the
                         // loop stops
        next_cluster = fat_table_get_next_cluster(file->table, last_cluster);
        fat_table_set_next_cluster(file->table, last_cluster, FAT_CLUSTER_FREE);
        last_cluster = next_cluster;
    ...
    file->dentry->base_name[0] =
        FAT_FILENAME_DELETED_CHAR; // FAT_FILENAME_DELETED_CHAR = 0xE5
    write_dir_entry(parent, file);
} /**/
